...
<body>
            ...
            <div class="container-fluid">
                <div class="row min-vh-100 flex-column flex-md-row">
                    ...
                    <main class="col-sm-10 bg-faded py-3 flex-grow-1">
                        <h2 class="page_head">Citizen View Details</h2>
                        <table id="citizen_view" class="table table-bordered table-hover dt-responsive"  >
                            ...
                            if($responseData != null){
                                foreach ($responseData  as $key => $value) {
                                    echo "<tr> <td>".  $value["fname"]. "</td><td> ". $value["lname"]. "</td><td> ". $value["nic"]. "</td><td> ". $value["age"]. "</td><td> "
                                        . $value["address"]. "</td><td> ". $value["profeson"]. "</td><td> ". $value["email"]. "</td><td> ". $value["affilication"]. "</td><td> "
                                        . $value["location"]. "</td><td> ". $value["date"]. "</td><td> ". $value["test"]. "</td><td> ". $value["results"]. "</td><td> "

                                    ?>
                                    <form action="controller/deleteCitizen.php" method="post">
                                        <input type="hidden" name="id" id="citId"  value="<?php echo $value["_id"];?>" >

                                            <button type="submit" name="submit" class="btn btn-danger">Deactivate</button>
                                        </td></tr>
                                    </form>
                                    <?php
    ...
    <script>
        ...
        function automagick(type, selItem, data, classes){
            var out = '';

            if (type=='select') {
                out = "<select><option>Select Status</option>" ;
                if (typeof classes !== 'undefined') {
                    out = '<select class="' + classes + '"><option>Select Status</option>' ;
                }

                for(var i = 0; i < data.length; ++i){
                    if(data[i] == selItem){
                        out += "<option selected value = '"+data[0]+"' >" + data[i] + "</option>";
                    }
                    else{
                        out += "<option  value = '"+data[0]+"' >" + data[i] + "</option>";
                    }
                }
                out += "</select>";
            } else if(type=='updatebutton') {
                out = "<button>Deactivate</button>" ;
                if (typeof classes !== 'undefined') {
                    out = '<button class="' + classes + '">Deactivate</button>' ;
                }
            } else if(type=='button') {
                out = "<button>" + selItem + "</button>";
                if (typeof classes !== 'undefined') {
                    out = '<button class="' + classes + '">' + selItem + '</button>' ;
                }
            } else {
                out = '!!!!!!!YOURSITEISHACKED!!!!!!!!';
            }
            return out;
        }
